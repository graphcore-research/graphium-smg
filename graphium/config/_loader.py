...
# Graphium
from graphium.utils.mup import set_base_shapes, save_base_shapes
from graphium.utils.spaces import DATAMODULE_DICT, GRAPHIUM_PRETRAINED_MODELS_DICT
from graphium.utils import fs

#from graphium.ipu.ipu_tensor_name_dump import TensorNameCallback
...
def load_predictor(
    ...
    # mup base shapes
    mup_base_path = config["architecture"].get("mup_base_path")

    if not isinstance(mup_base_path, str):
        raise ValueError("mup_base_path must be provided and must be str")
    
    mup_load_or_save = config["architecture"].get("mup_load_or_save")

    if mup_load_or_save not in ['load','save']:
        raise ValueError("mup_load_or_save must be provided and must be one of 'load' or 'save'")
    
    predictor = load_mup(mup_base_path, mup_load_or_save, predictor)
...
def load_mup(
    ...
    # TODO: Check what rescale_params should be
    if mup_load_or_save == 'save':
        base = model.__class__(**model.make_mup_base_kwargs(divide_factor=2))
        predictor.model = set_base_shapes(model, base)
        save_base_shapes(model, mup_base_path)

    elif mup_load_or_save == 'load':
        predictor.model = set_base_shapes(model, mup_base_path)
