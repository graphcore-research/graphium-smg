...
def train_one_epoch(model, dataloader, loss_fn, optimizer, task_type, epoch, fold):
    model.train()
    total_loss = 0
    for (graph_inputs, node_inputs), targets in dataloader:
        # Filter samples with NaNs out
        nan_mask = ~torch.isnan(graph_inputs).any(dim=1)
        filtered_inputs = graph_inputs[nan_mask]
        filtered_targets = targets[nan_mask]
        filtered_node_inputs = Batch.from_data_list(node_inputs[nan_mask])

        if len(filtered_inputs) > 0:
            optimizer.zero_grad()
            outputs = model(filtered_inputs.float(), filtered_node_inputs)
...
def evaluate(model, dataloader, loss_fn, task_type, evaluation_type, epoch, fold):
    ...
    with torch.no_grad():
        for (graph_inputs, node_inputs), targets in dataloader:
            outputs = model(graph_inputs.float(), node_inputs)
    ...
    def __init__(self, input_dim, depth=3, hidden_dim=512, activation_fn='relu', combine_input='concat', num_classes=None, dropout_rate=0.1, **kwargs):
        ...
        for i in range(depth):
            if i == 0: # first layer
                ...
                in_dim = 2*input_dim + hidden_dim if self.combine_input == 'concat' else hidden_dim
        ...
        node_feat_dim = 512
        edge_feat_dim = 64
        gconv_output_dim_divisor = 2
        edge_mlp_hidden_dim = 15
        edge_mlp = nn.Sequential(
            nn.Linear(edge_feat_dim, edge_mlp_hidden_dim),
            nn.ReLU(),
            nn.Linear(edge_mlp_hidden_dim, node_feat_dim * hidden_dim // gconv_output_dim_divisor)
        )
        self.gconv1 = NNConv(node_feat_dim, hidden_dim // gconv_output_dim_divisor, edge_mlp)
        self.gconv2 = NNConv(hidden_dim // gconv_output_dim_divisor, hidden_dim // gconv_output_dim_divisor, edge_mlp)
        self.gmlp1 = nn.Linear(hidden_dim // gconv_output_dim_divisor, input_dim)


    def forward(self, x, x2):
        original_x = x
        node_feat, edge_index, edge_feat = x2.feat, x2.edge_index, x2.edge_feat

        node_feat = F.relu(self.gconv1(node_feat, edge_index, edge_feat))
        node_feat = F.relu(self.gconv2(node_feat, edge_index, edge_feat))
        graph_feat = global_mean_pool(node_feat, x2.batch)
        graph_feat = F.relu(self.gmlp1(graph_feat))
        ...
        for i in range(self.depth):
            ...
            if self.combine_input == 'concat' and i == self.depth - 2:
                x = torch.cat((x, original_x, graph_feat), dim=1)
...
def dataloader_factory(split_name, group, benchmark, i2v, args, seed=42):
    ...
    def match_and_replace_input_column(samples_df):
        ...
        transformed_df["graph_feat"] = transformed_df['Drug'].apply(
            lambda s: i2v[dm.unique_id(s)]['graph_feat'].detach().numpy())
        transformed_df["node_feat"] = transformed_df['Drug'].apply(
            lambda s: i2v[dm.unique_id(s)]['node_feat'])
            
        return transformed_df

    class SingleInstancePredictionDataset(Dataset):
        def __init__(self, samples_df, task_type):
            self.graph_feat = samples_df['graph_feat'].tolist()
            self.node_feat = samples_df['node_feat'].tolist()
        ...
        def __getitem__(self, idx):
            graph_feat = torch.tensor(self.graph_feat[idx])
            node_feat = self.node_feat[idx]
            target = torch.tensor(self.targets[idx])
            return graph_feat, node_feat, target

    def collate(samples):
        graph_feats, node_feats, targets = zip(*samples)
        # Convert list of Data objects to a single Batch object
        node_feats_batch = Batch.from_data_list(list(node_feats))

        # Convert graph_feats and targets to tensors
        graph_feats_batch = torch.stack(graph_feats)
        targets_batch = torch.stack(targets)

        return (graph_feats_batch, node_feats_batch), targets_batch
    ...
    if split_name == 'train+val':
        ...
        val_loader = DataLoader(val_dataset, batch_size=args.batch_size, collate_fn=collate, shuffle=False)
    else:
        ...
        test_loader = DataLoader(test_dataset, batch_size=args.batch_size, collate_fn=collate, shuffle=False)

        input_dim = test_samples['graph_feat'].iloc[0].shape[0]
...
def main():
    ...
    print(f"The fingerpints have the following features: {i2v[list(i2v.keys())[0]].keys()}")
    print(f"Within 'node_feat' there are the following features: {i2v[list(i2v.keys())[0]]['node_feat']}")
