...
pickle_path = 'results/sweep_results_dict_2.pickle'
...
MODELS = [
    # 'GINE_AdmetFiltered_k1',
    # 'GINE_AdmetFiltered_k2',
    # 'GINE_AdmetFiltered_k10',
    'GINE_AdmetFiltered_maxpool',
    # 'GINE_AdmetFiltered_meanpool',
    # 'GINE_AdmetFiltered_seed11',
    # 'GINE_AdmetFiltered_seed6',
    # 'agbt_baseline_ss=18',
    # 'bet_baseline_ss=18',
    # 'molformer_baseline_ss=18'
    # 'mpnn_baseline_ss=432',
    # 'gcn_baseline_ss=18',
    # 'gine_baseline_ss=18',
    # 'mpnn_baseline_ss=18',
    # 'gcn_baseline_ss=5',
    # 'gine_baseline_ss=5',
    # 'mpnn_baseline_ss=5',
    # 'gcn_baseline_ss=5_lastnorm',
    # 'mpnn_baseline_ss=5_lastnorm'
    # 'GCN_AdmetFiltered_ensemble_10M',
    # 'GINE_AdmetFiltered_ensemble_10M',
    # 'GINE_AdmetFiltered_lastnorm_ensemble_10M',
    # 'GINE_filtered_pcba',
    # 'GINE_filtered_pcba_lr3e-4_40M',
    # 'GINE_filtered_pcba_lr4e-4_40M'
]
...
def find_best_score_for_sweep(sweep):
    mean_val_scores, std_val_scores, run_indices = [], [], []
    ...
    for idx, run in enumerate(sweep.runs):
        ...
        if "statistics" in run.summary_metrics.keys():
            ...
            if f"{metric}" in run_statistics.keys():
                ...
                std_val_scores += [run_statistics[metric]['std']]
                run_indices += [idx]
    ...
    if best_mean_test_score != 'NaN':
        for score, idx in zip(mean_val_scores, run_indices):
            if score == best_mean_test_score:
                index_of_best_score = idx
        ...
        run = sweep.runs[index_of_best_score]
        print('python dist.py ' + ' '.join(run.metadata['args']) + ' --wandb-off')
        print()
    else:
        best_std_test_score = 'NaN'
        best_mean_test_score = 'NaN'
