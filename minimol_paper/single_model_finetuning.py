...
def main():
    ...
    for _ in range(repeat):
        ...
        for seed, fold in zip(SEEDS, range(args.num_cross_validation_folds)):
            ...
            best_results += [deepcopy(best_epoch['val_results'])]
            results = aggregate_dicts(dicts=[results, best_epoch['val_results']])

        # Test trained model
        print(best_results)
        val_losses = [res['val_loss'] for res in best_results]
        index_of_lowest_val_loss = val_losses.index(min(val_losses))
        best_model = best_models[index_of_lowest_val_loss]
        
        test_results = ensemble_eval([best_model], test_dl, loss_fn, args.task_type, evaluation_type='test')
